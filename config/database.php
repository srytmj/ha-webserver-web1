...
define('DB_HOST',          '[DB_PRIMARY_PRIVATE_IP]'); // MySQL on EC2 = PRIMARY (sumber data live)
...
// Guard operasi tulis: saat aktif di fallback RDS (read-only), tolak dengan 503 yang rapi
// alih-alih melempar stack trace mentah. Dipanggil di awal handler create/update/delete.
function assertWritable(): void {
    if (!defined('DB_READONLY')) {
        getDBConnection(); // pastikan deteksi node sudah berjalan
    }
    if (defined('DB_READONLY') && DB_READONLY) {
        http_response_code(503);
        die('<h2 style="font-family:sans-serif;color:#C0392B;padding:32px;">'
          . '503 — Database sedang dalam mode read-only (failover ke salinan RDS). '
          . 'Operasi tulis tidak tersedia sementara hingga node primary pulih.</h2>');
    }
}
